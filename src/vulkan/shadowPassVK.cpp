...
ShadowPassVK::ShadowPassVK(
    ...
    const ImageBlock& i_out_shadow_attachment) :
    RenderPassVK(i_runtime),
    m_out_shadow_attachment(i_out_shadow_attachment)
{
...
VkCommandBuffer ShadowPassVK::draw(const Frame& i_frame)
{
    ...
    UtilsVK::beginRegion(current_cmd, "Shadow Pass Pass", Vector4f(0.0f, 0.5f, 0.0f, 1.0f));
...
void ShadowPassVK::createFbo()
{
    ...
    for (size_t i = 0; i < m_fbos.size(); i++)
    {
        ...
        attachments[0] = m_out_shadow_attachment.m_image_view;     // Shadow depth attachment
        ...
        framebuffer_create_info.layers = MAX_SHADOW_LAYERS; // Support multiple layers for shadow mapping

        if (vkCreateFramebuffer(renderer.getDevice()->getLogicalDevice(), &framebuffer_create_info, nullptr, &m_fbos[i]))
        {
            throw MiniEngineException("failed to create shadow pass fbos");
        }
    }
}
